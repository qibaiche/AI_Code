...
本工具实现以下自动化流程：
1. 📖 读取 LOT 列表文件
2. 🔄 **自动处理 SQLPathFinder 更新对话框**（如需要）
3. 🚀 自动启动 SQLPathFinder 并执行 VG2 脚本
4. 📊 等待数据抓取完成
5. 📈 生成 Functional_bin × DevRevStep 的 Pareto 分析表
6. 📧 自动发送 Excel 报表邮件
...
**问题**：当隔一段时间后运行，SQLPathFinder 会弹出 "Update Recommended" 对话框

**解决方案**：
✅ **工具已自动处理**：从 v1.1.1 版本开始，工具会自动：
1. 检测更新对话框
2. 自动点击 "Yes" 按钮
3. 运行 `StartSQLPathFinder.bat` 触发升级
4. 等待升级完成（最多 90 秒）
5. 关闭并重新打开 SQLPathFinder
6. 继续自动化流程

**配置升级脚本路径**：
在 `prd_lot_automation/config.yaml` 中配置：
```yaml
paths:
  upgrade_script: "C:/Users/你的用户名/My Programs/SQLPathFinder3/StartSQLPathFinder.bat"
```

**注意事项**：
- 升级过程可能需要 60-90 秒，请耐心等待
- 如果升级脚本路径不对，请在配置文件中修改
- 升级完成后，工具会自动继续执行

**注意事项**：
- 如果自动处理失败，请查看日志文件了解详情
- 升级过程通常需要几秒钟，请耐心等待
...
**更新日志**：
- v1.1.3 (2025-12-16): 优化升级流程，简化代码，升级等待时间改为 5 秒
- v1.1.2 (2025-12-16): 修复升级流程，点击 Yes 后自动运行 StartSQLPathFinder.bat 触发升级
- v1.1.1 (2025-12-16): 修复更新对话框处理，改为直接点击 Yes 按钮（不使用 Enter 键）
- v1.1 (2025-11-18): 新增自动处理 SQLPathFinder 更新对话框功能
